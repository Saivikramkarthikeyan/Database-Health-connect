...
MODIFY COLUMN type_id INT NOT NULL DEFAULT 1;
...
-- 4th Procedure (Handles payment processing and insurance claims) (Udhva Patel)
DELIMITER //
...
CREATE PROCEDURE ProcessPayment(
    ...
    IN p_insurance_claim_id VARCHAR(50),
    OUT p_result VARCHAR(255)
)
BEGIN
    DECLARE v_total_amount DECIMAL(10,2);
    DECLARE v_current_paid DECIMAL(10,2);
    DECLARE v_current_insurance DECIMAL(10,2);
    DECLARE v_patient_id INT;
    DECLARE v_insurance_provider_id INT;
    
    -- Get current payment details
    SELECT amount, amount_paid, insurance_covered, patient_id 
    INTO v_total_amount, v_current_paid, v_current_insurance, v_patient_id
    FROM Payments 
    WHERE appointment_id = p_appointment_id;
    
    -- Get patient's insurance provider
    SELECT insurance_provider_id INTO v_insurance_provider_id
    FROM Patients WHERE patient_id = v_patient_id;
    
    -- Validate payment amount
    IF p_payment_amount <= 0 THEN
        SET p_result = 'Error: Payment amount must be positive';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Payment amount must be positive';
    END IF;
    
    -- Check if payment exceeds balance
    IF p_payment_amount > (v_total_amount - v_current_paid - v_current_insurance) THEN
        SET p_result = 'Error: Payment exceeds outstanding balance';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Payment exceeds outstanding balance';
    END IF;
    
    -- Process payment differently based on whether it's insurance or patient payment
    IF p_payment_method = 'Insurance' THEN
        -- For insurance payments, add to insurance_covered
        UPDATE Payments 
        SET insurance_covered = insurance_covered + p_payment_amount,
            payment_date = NOW(),
            payment_method = p_payment_method,
            insurance_claim_id = p_insurance_claim_id,
            status = CASE 
                WHEN (amount_paid + insurance_covered + p_payment_amount) >= amount THEN 'Completed'
                ELSE 'Partial'
            END
        WHERE appointment_id = p_appointment_id;
    ELSE
        -- For patient payments, add to amount_paid
        UPDATE Payments 
        SET amount_paid = amount_paid + p_payment_amount,
            payment_date = NOW(),
            payment_method = p_payment_method,
            status = CASE 
                WHEN (amount_paid + p_payment_amount + insurance_covered) >= amount THEN 'Completed'
                WHEN (amount_paid + p_payment_amount + insurance_covered) > 0 THEN 'Partial'
                ELSE 'Pending'
            END
        WHERE appointment_id = p_appointment_id;
    END IF;
    
    SET p_result = CONCAT('Payment processed successfully. New balance: ', 
                         (v_total_amount - v_current_paid - p_payment_amount - v_current_insurance));
END //

DELIMITER ;

-- to use 4th procedure 

CALL ProcessPayment(
    11,                 -- Appointment ID
    25.00,             -- Payment amount
    'Credit Card',      -- Payment method
    NULL,               -- Insurance claim ID 
    @payment_result     -- Output variable
);

-- For to use Insurance as payment
CALL ProcessPayment(
    15,                 -- Appointment ID
    15.00,              -- Insurance payment amount
    'Insurance',        -- Must specify 'Insurance' as method
    'KVT-2023-98769',   -- Insurance claim ID
    @insurance_result
);



-- 5th Procedure (Creates financial/Billing reports by time period) (Udhva Patel)
DELIMITER //

CREATE PROCEDURE GenerateBillingReport(
    IN p_start_date DATE,
    IN p_end_date DATE,
...
BEGIN
    SELECT 
        ...
        SUM(p.insurance_covered) AS total_insurance,
        (SUM(p.amount) - SUM(p.amount_paid) - SUM(p.insurance_covered)) AS outstanding_balance,
        AVG(p.amount) AS average_bill_amount
    FROM 
        Payments p
    ...
    WHERE 
        ...
        AND (p_department_id IS NULL OR doc.department_id = p_department_id)
    GROUP BY 
        d.department_name
    ORDER BY 
        total_billed DESC;
...
-- 1st Function (Computes age from DOB) (Udhva Patel)
...
BEGIN
    ...
    SELECT dob INTO v_dob FROM Patients WHERE patient_id = p_patient_id;
...
-- 2nd Function (Standardizes phone display) (Udhva Patel)
DELIMITER //

CREATE FUNCTION FormatPhoneNumber(p_phone VARCHAR(15)) 
RETURNS VARCHAR(20)
DETERMINISTIC
BEGIN
    RETURN CONCAT('(', SUBSTRING(p_phone, 1, 3), ') ', 
           SUBSTRING(p_phone, 4, 3), '-', SUBSTRING(p_phone, 7, 4));
END //

DELIMITER ;

-- 3rd Function 
-- Check payment needed for a prior visit (Wu)

DELIMITER //
CREATE FUNCTION GetTotalPaymentsForAppointment(appId INT)
...
SELECT GetTotalPaymentsForAppointment(3);

-- 4th Function (Wu)
-- When did patient came last time?

DELIMITER //

CREATE FUNCTION GetPatientLastVisit(patient_id_param INT) 
RETURNS DATETIME
DETERMINISTIC
READS SQL DATA
BEGIN
    ...
    SELECT MAX(appointment_date) INTO last_visit
    FROM Appointments
...
SELECT GetPatientLastVisit(9) AS last_visit_date;



-- 5th Function (Wu)
-- check how many appointment for a doctor they have for a specific day
DELIMITER //
CREATE FUNCTION GetAppointmentCountForDoctorOnDate(doctorId INT, targetDate DATE)
RETURNS INT
DETERMINISTIC
BEGIN
    DECLARE total INT;
    ...
    WHERE doctor_id = doctorId
      AND DATE(appointment_date) = targetDate
...
SELECT GetAppointmentCountForDoctorOnDate(2, '2025-03-22');



-- Triggers

-- 1st Trigger (Prevent Overbooking a Room Based on Capacity) (Sai Vikram)
DELIMITER $$
...
DELIMITER ;
-- Attempt to insert a new appointment
INSERT INTO Appointments (appointment_id, patient_id, doctor_id, room_id, appointment_date, end_time, status, type_id)
VALUES (101, 5, 2, 3, '2025-04-21 10:00:00', '2025-04-21 11:00:00', 'Scheduled', 1);
...
-- 2nd Trigger (Prevent Duplicate Allergy Entries Avoid inserting the same allergy twice for a patient.) (Sai Vikram)
DELIMITER $$
...
-- Valid insert
INSERT INTO patientallergies (patient_id, allergy_name) 
VALUES (1, 'Peanuts');

-- This will trigger an error if the above entry already exists
INSERT INTO patientallergies (patient_id, allergy_name) 
VALUES (1, 'Peanuts');
...
-- 3rd Trigger (Auto-assign Default Doctor for Appointments) (Sai Vikram)

DELIMITER //

CREATE TRIGGER auto_assign_doctor
BEFORE INSERT ON Appointments
FOR EACH ROW
BEGIN
    ...
    DECLARE specialization_pattern VARCHAR(100);
    
    -- Only proceed if doctor isn't already assigned
    IF NEW.doctor_id IS NULL THEN
        -- Get appointment duration
        SELECT at.duration_minutes INTO duration_minutes
        FROM AppointmentTypes at
        WHERE at.type_id = NEW.type_id;
        
        -- Calculate end time if not provided
        IF NEW.end_time IS NULL THEN
            SET NEW.end_time = DATE_ADD(NEW.appointment_date, INTERVAL duration_minutes MINUTE);
        END IF;
        
        -- Determine likely department based on appointment type
        SELECT 
            CASE 
                WHEN at.type_name LIKE '%Cardiology%' THEN 1
                WHEN at.type_name LIKE '%Neurology%' THEN 2
                WHEN at.type_name LIKE '%Pediatrics%' THEN 3
                WHEN at.type_name LIKE '%Orthopedic%' THEN 4
                WHEN at.type_name LIKE '%Oncology%' THEN 5
                WHEN at.type_name LIKE '%X-ray%' OR at.type_name LIKE '%MRI%' THEN 6
                WHEN at.type_name LIKE '%Lab%' THEN 7
                ELSE 1 -- Default to first department
            END INTO department_id_var
        FROM AppointmentTypes at
        WHERE at.type_id = NEW.type_id;
        
        -- Create a pattern to match doctor specialization
        SELECT 
            CASE 
                WHEN at.type_name LIKE '%Cardiology%' THEN 'Cardiolog%'
                WHEN at.type_name LIKE '%Neurology%' THEN 'Neurolog%'
                WHEN at.type_name LIKE '%Pediatrics%' THEN 'Pediatri%'
                WHEN at.type_name LIKE '%Orthopedic%' THEN 'Orthoped%'
                WHEN at.type_name LIKE '%Oncology%' THEN 'Oncolog%'
                WHEN at.type_name LIKE '%X-ray%' OR at.type_name LIKE '%MRI%' THEN 'Radiolog%'
                ELSE '%' -- Match any specialization
            END INTO specialization_pattern
        FROM AppointmentTypes at
        WHERE at.type_id = NEW.type_id;
        
        -- Find an available doctor
        SELECT d.doctor_id INTO doctor_id_var
        FROM Doctors d
        WHERE d.department_id = department_id_var
        AND d.specialization LIKE specialization_pattern
        AND d.doctor_id NOT IN (
            -- Doctors with conflicting appointments
            SELECT a.doctor_id
            FROM Appointments a
            WHERE a.appointment_date < NEW.end_time
            AND a.end_time > NEW.appointment_date
            AND a.status != 'Cancelled'
        )
        AND d.doctor_id NOT IN (
            -- Doctors on time off
            SELECT dt.doctor_id
            FROM DoctorTimeOff dt
            WHERE NEW.appointment_date BETWEEN dt.start_date AND dt.end_date
            AND dt.is_approved = TRUE
        )
        AND EXISTS (
            -- Doctors available at this time according to their schedule
            SELECT 1
            FROM DoctorSchedules ds
            WHERE ds.doctor_id = d.doctor_id
            AND ds.day_of_week = DAYNAME(NEW.appointment_date)
            AND TIME(NEW.appointment_date) >= ds.start_time
            AND TIME(NEW.end_time) <= ds.end_time
            AND (ds.valid_to IS NULL OR ds.valid_to >= DATE(NEW.appointment_date))
            AND ds.is_recurring = TRUE
        )
        ORDER BY (
            -- Prefer doctors with the fewest appointments that day
            SELECT COUNT(*) 
            FROM Appointments a 
        ...
        -- If we found a doctor, assign them
        IF doctor_id_var IS NOT NULL THEN
            SET NEW.doctor_id = doctor_id_var;
            
            -- Try to find an available room in the same department
            SELECT r.room_id INTO room_id_var
            FROM Rooms r
            WHERE r.department_id = department_id_var
            ...
            IF room_id_var IS NOT NULL THEN
                SET NEW.room_id = room_id_var;
            END IF;
        END IF;
    END IF;
END //
...
-- to auto assign doctor
INSERT INTO Appointments (patient_id, type_id, appointment_date, status)
VALUES (2, 4, '2023-06-22 14:30:00', 'Scheduled');



-- 4th Trigger (pranav vanam)
...
BEGIN
  ...
  IF conflict_count > 0 THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'This Appointment already exist.';
  END IF;
END$$

DELIMITER ;

-- Prevent same booking
INSERT INTO Appointments (doctor_id, patient_id, appointment_date, end_time, status)
VALUES (1, 1, '2025-05-01 11:00:00', '2025-05-01 12:00:00', 'Scheduled');

SELECT * FROM Patients;

-- 5th Trigger (pranav vanam)
...
-- To use this trigger, which book appointment in past and shows error
INSERT INTO Appointments (doctor_id, patient_id, appointment_date, status)
VALUES (2, 104, '2025-04-01 09:00:00', 'Scheduled');
